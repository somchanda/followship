        ...
        <div class="list-group following_you clearfix">
            <div class="list-group-item d-inline-block">
                <div class="user_profile_image" style=" background : url('{{asset("assets/img/users/".$item2->user2->avatar)}}');
                    ...
                    margin-right: 20px;
                    "></div>
                <div class="followingship-username float-left">{{$item2->user2->name}}</div>
                <button class="float-right followed followingship-status" onclick="processData('{{$item2->user2->id}}','unfollowing')">Followed</button>
            </div>
        </div>
    @endforeach
@else
